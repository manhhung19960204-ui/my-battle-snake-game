...
#include "app_data.h"
#include "app.h"
#include "scr_game.h"   // để chơi lại (SCREEN_GOTO &scr_game)
...
static void view_scr_gameover() {
    ...
    // Canh giữa tương đối: font mặc định ~6px/ký tự, khung rộng 118px
    int title_len = 0;
    ...
    if (app_data.mode == GAME_MODE_COM) {
        // Hiện cả 2 điểm, có nhãn YOU / COM để phân biệt
...
void scr_gameover_handle(ak_msg_t *msg) {
    ...
    case AC_DISPLAY_BUTON_DOWN_PRESSED: {
        // Chơi lại (cho phép cả 2 nút để tiện thao tác)
        SCREEN_TRAN(scr_game_handle, &scr_game);
    } break;

    case AC_DISPLAY_BUTON_MODE_PRESSED: {
        // Về menu chính (giống cách scr_game dùng SCREEN_BACK)
        SCREEN_BACK();
    } break;
